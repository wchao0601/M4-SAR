...
class yolov8_heatmap:
    ...
    def process(self, img_path, img_path1, save_path):
        # img process
        ...
        img1 = letterbox(img1)[0]
        img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2RGB)
        img1 = np.float32(img1) / 255.0
        tensor1 = torch.from_numpy(np.transpose(img1, axes=[2, 0, 1])).unsqueeze(0).to(self.device)

        x = torch.cat((tensor, tensor1), axis=1)
        try:
            grayscale_cam = self.method(x, [self.target])
    ...
    def __call__(self, img_path, img_path1, save_path):
        ...
        if os.path.isdir(img_path):
            ...
            self.process(img_path, img_path1, f'{save_path}/'+file_id)
        
def get_params():
    params = {
        'weight': 'runs/predict/M4-SAR/yolo11-obb-E2E-OSDet-300e/weights/best.pt', # weight
        'device': 'cuda:0',
...
if __name__ == '__main__':
    # gen heatmap on optical image
    model = yolov8_heatmap(**get_params())
    source1_path = "M4-SAR/optical/test/images"
    source2_path = "M4-SAR/sar/test/images"
    filelist = os.listdir(source1_path)
    for file in filelist:
        file_id = file.split('.')[0]
        model(source1_path+file, source2_path+file, 'runs/predict/M4-SAR/yolo11-obb-E2E-OSDet-300e/optical/heatmap')
