...
def generate_template_mask(bs, len_template_feat, number):
    template_feat_size = int(len_template_feat**0.5)
    ...
    box_mask_z = torch.zeros([bs, template_feat_size, template_feat_size], device='cuda')
    box_mask_z[:, index, index] = 1
    box_mask_z = box_mask_z.flatten(1).to(torch.bool)
    box_mask_z_new = box_mask_z
    for i in range(number):
        if i == 0:
            continue
        else:
            box_mask_z_new = torch.cat([box_mask_z_new,box_mask_z],dim=1)

    return box_mask_z_new
