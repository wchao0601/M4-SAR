...
from lib.models.sttrack.base_backbone import BaseBackbone
from lib.models.sttrack.utils import combine_tokens, recover_tokens, generate_template_mask
...
class VisionTransformer(BaseBackbone):
    ...
    def forward_features(self, z, x,track_query_before,keep_rate):
        # keep_rate = [1.0,0.85,0.7]
        self.keep_rate = keep_rate
        keep_layer = self.depth//len(keep_rate)
        ...
        token_number = x_r.size()[1]    
        lens_z = self.pos_embed_z.shape[1]
        lens_x = self.pos_embed_x.shape[1]

        template_mask = generate_template_mask(B, lens_z, number)    
        lens_z = lens_z*number
        # print(track_query_before[0].size()[0])
        if track_query_before[0].size()[0] != B:
            track_query_r = track_query_before[0].expand(B,-1,-1)
            track_query_x = track_query_before[1].expand(B,-1,-1)
        else:
            track_query_r = track_query_before[0]
            track_query_x = track_query_before[1]

        x_r = torch.cat([x_r, track_query_r],dim=1)
        x_x = torch.cat([x_x, track_query_x],dim=1)
    
      
        for i, blk in enumerate(self.blocks):
            x_r, attn_r = blk(x_r, return_attention=True)
            x_x, attn_x = blk(x_x, return_attention=True)
            _,N,_ = x_x.shape
            if i < self.depth:
                # BSI 
                if int(self.keep_rate[i//keep_layer]) == 1:
                    query_r = x_r
                    query_x = x_x
                else:
                    query_r = candidate_elimination(x_r, attn_r, template_mask, track_query_before[0].size()[1],lens_z,self.keep_rate[i//keep_layer])
                    query_x = candidate_elimination(x_x, attn_x, template_mask, track_query_before[0].size()[1],lens_z,self.keep_rate[i//keep_layer])
                
                concat_cls = torch.cat([query_r,query_x],dim=-1)
                cross_features = self.BSI[i](concat_cls)
                x_r = cross_features + x_r

                concat_cls = torch.cat([query_x,query_r],dim=-1)
                cross_features = self.BSI[i](concat_cls)
                x_x = cross_features + x_x
