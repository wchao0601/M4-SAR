...
class STTRACKActor(BaseActor):
    ...
    def forward_pass(self, data):
        ...
        for i in range(self.settings.num_template):
            template_img = data['template_images'][i].view(-1,
                                                             *data['template_images'].shape[2:])  # (batch, 6, 128, 128)
            template_list.append(template_img)

        for i in range(self.settings.num_search):
            search_img = data['search_images'][i].view(-1,
                                                             *data['search_images'].shape[2:])  # (batch, 6, 128, 128)
        ...
        keep_rate = adjust_keep_rate(data['epoch'], warmup_epochs=drop_start_epoch,
