...
# # 自定义实现
# def smooth_l1_loss_custom(y_true, y_pred, beta=1.0):
#     diff = torch.abs(y_true - y_pred)
#     return torch.mean(torch.where(diff < beta, 0.5 * diff**2 / beta, diff - 0.5 * beta))

def pat_loss(y_s, y_t, tau=1.0, path_size=1):
    """Forward computation.
    Args:
    ...
    for idx, (s, t) in enumerate(zip(y_s, y_t)):
        ...
        pad = 0
        if H % path_size != 0:
            while (H + pad) % path_size != 0:
                pad = pad + 1
            padding = (0, pad, 0, pad)
            s = F.pad(s, padding, mode='constant', value=0)
            t = F.pad(t, padding, mode='constant', value=0)
        device = s.device
        softmax_pred_T = F.softmax(t.view(N*C, (H+pad)*(W+pad)//path_area, path_size, path_size) / tau, dim=1)  # [N*C, H*W]
        logsoftmax = torch.nn.LogSoftmax(dim=1)
        cost = torch.sum(
            softmax_pred_T * logsoftmax(t.view(N*C, (H+pad)*(W+pad)//path_area, path_size, path_size) / tau) - 
            softmax_pred_T * logsoftmax(s.view(N*C, (H+pad)*(W+pad)//path_area, path_size, path_size) / tau)) * (tau ** 2)#*scale_factor #* scale_factor.abs().pow(1.0).sum()
