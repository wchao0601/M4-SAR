...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        if img.shape[2]==3:
            ...
            if self.hgain or self.sgain or self.vgain:
                r = np.random.uniform(-1, 1, 3) * [self.hgain, self.sgain, self.vgain] + 1  # random gains
                hue1, sat1, val1 = cv2.split(cv2.cvtColor(im1, cv2.COLOR_BGR2HSV))
                dtype1 = im1.dtype  # uint8

                hue2, sat2, val2 = cv2.split(cv2.cvtColor(im2, cv2.COLOR_BGR2HSV))
                # dtype2 = im2.dtype  # uint8

                x = np.arange(0, 256, dtype=r.dtype)
                lut_hue = ((x * r[0]) % 180).astype(dtype1)
                lut_sat = np.clip(x * r[1], 0, 255).astype(dtype1)
                lut_val = np.clip(x * r[2], 0, 255).astype(dtype1)

                im_hsv1 = cv2.merge((cv2.LUT(hue1, lut_hue), cv2.LUT(sat1, lut_sat), cv2.LUT(val1, lut_val)))
                im1 = cv2.cvtColor(im_hsv1, cv2.COLOR_HSV2BGR)

                im_hsv2 = cv2.merge((cv2.LUT(hue2, lut_hue), cv2.LUT(sat2, lut_sat), cv2.LUT(val2, lut_val)))
                im2 = cv2.cvtColor(im_hsv2, cv2.COLOR_HSV2BGR)
                img = np.concatenate([im1, im2], axis=2) # wchao end
        return labels
...
class LetterBox:
    ...
    def __init__(self, new_shape=(640, 640), auto=False, scaleFill=False, scaleup=True, center=True, stride=0):
        """
    ...
    def __call__(self, labels=None, image=None):
        ...
        if shape[::-1] != new_unpad:  # resize
            if img.shape[2]==6: # wchao begin 
                ...
                img = cv2.resize(img, new_unpad, interpolation=cv2.INTER_LINEAR)  # wchao end 
        top, bottom = int(round(dh - 0.1)) if self.center else 0, int(round(dh + 0.1))
        left, right = int(round(dw - 0.1)) if self.center else 0, int(round(dw + 0.1))
        # wchao begin
        if img.shape[2]==6:
            im1 = cv2.copyMakeBorder(
            im1, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        ) # add border
            im2 = cv2.copyMakeBorder(
            im2, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        ) # add border
            img = np.concatenate([im1, im2], axis=2)
        else:
            img = cv2.copyMakeBorder(
            img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
        ) # add border # wchao end
